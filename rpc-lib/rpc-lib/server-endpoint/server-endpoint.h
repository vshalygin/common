...
namespace vshalygin::rpc {
    ...
    {
        using connection_change_state_handler_t =
    ...
    template<typename Request, typename Response>
    server_endpoint::response_results_t<Response> server_endpoint::make_request_all
        (const Request &req, const auto &create_stub, auto method)
    {
    ...
    template<typename Response>
    server_endpoint::request_callback_t<Response>
        server_endpoint::create_sync_response_callback
        (std::shared_ptr<req_result_data<Response>> result_data,
         std::shared_ptr<cl::latch> latch)
    {
