...
TEST(PipeEnvironment, StopWaitingAndOpensPipeWhenItIsCreated)
{
    event sync_event1;
    event sync_event2;
    auto pool = std::make_shared<thread_pool>(4);
    auto sut = pipe_environment::create(pool);
    std::shared_ptr<pipe_endpoint> client_enpoint;
    pool->post([&]() {
        sync_event1.set();
        client_enpoint = sut->open_pipe_timed(s_pipe_name, std::chrono::seconds(60));
        sync_event2.set();
    });
...
TEST(PipeEnvironment, StopWaitingAndOpensPipeWhenPreviousEndpointDestroyed)
{
    event sync_event1;
    event sync_event2;
    auto pool = std::make_shared<thread_pool>(4);
    ...
    auto server_endpoint = sut->create_pipe(s_pipe_name);
    auto client_enpoint = sut->open_pipe(s_pipe_name);
    std::shared_ptr<pipe_endpoint> client_enpoint2;
    pool->post([&]() {
        sync_event1.set();
        client_enpoint2 = sut->open_pipe_timed(s_pipe_name, std::chrono::seconds(60));
        sync_event2.set();
    });
    ...
    ASSERT_TRUE(sync_event2.wait_for(std::chrono::seconds(10)));
    ASSERT_TRUE(client_enpoint2->is_connected());
}
